...
public class MatrixChainMultiplicationRecursive {
    ...
    public static int matrixChainMultiplication(int[] arr, int low, int high) {
        ...
        for (int k = low; k <= high - 1; k++) {
            int iToK = matrixChainMultiplication(arr, low, k);
            int k1ToJ = matrixChainMultiplication(arr, k + 1, high);
            //log.debug("arr[{}] * arr[{}] * arr[{}] ==> {} * {} * {}", low - 1, k, high, arr[low - 1], arr[k], arr[high]);
            int temp = arr[low - 1] * arr[k] * arr[high];

            int tempAnswer = iToK + k1ToJ + temp;
            //log.debug("iToK + k1ToJ + temp => {} + {} + {} = {}", iToK, k1ToJ, temp, tempAnswer);
